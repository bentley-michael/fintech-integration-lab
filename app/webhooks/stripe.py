...
def verify_signature(payload: bytes, sig_header: str, secret: str) -> bool:
    """
    Verifies the Stripe-Signature header manually (HMAC-SHA256).
...
async def stripe_webhook(request: Request, stripe_signature: Optional[str] = Header(default=None, alias="Stripe-Signature")):
    ...
    valid = verify_signature(body_bytes, stripe_signature, webhook_secret)
    if not valid:
        logger.warning("Invalid Stripe signature verification")
